...
static const int api_poll_ms = 300000; // 5分钟
...
void control_loop_task(void *arg) {
    ...
    while (1) {
        ...
        if (now - last_log_time > 10000) { // 每10秒记录一次日志
            ESP_LOGI("CONTROL", "Mode: %d, Servo: %.1f, Stepper: %.1f", 
                    g_mode_status, servo.current_angle, motor.current_angle);
            last_log_time = now;
        }
...
void servo_init(ServoControl *s, int pin, float max_speed) {
    ...
    ESP_LOGI("SERVO", "Servo initialized on pin %d", pin);
}
...
void motor_task(void *arg) {
    ...
    while (1) {
        ...
        if (now - last_log > MOTOR_LOG_INTERVAL_MS) {
            ESP_LOGI("MOTOR", "Stepper: current=%.1f°, target=%.1f°, steps=%ld", 
                    motor.current_angle, motor.target_angle, motor.current_step);
            last_log = now;
        }
