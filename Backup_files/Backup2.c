...
static int characteristic_write_cb(uint16_t conn_handle, uint16_t attr_handle, 
    ...
    if (ctxt->op == BLE_GATT_ACCESS_OP_WRITE_CHR) {
        ...
        stepper_move_to_angle(&motor, angle);
                
        ESP_LOGI(TAG_BLE, "Received angle command: %.2f°", angle);
    }
    return 0;
}
...
static int characteristic_read_cb(uint16_t conn_handle, uint16_t attr_handle,
    ...
    if (ctxt->op == BLE_GATT_ACCESS_OP_READ_CHR) {
        char buf[16];
        int len = snprintf(buf, sizeof(buf), "%.2f", motor.current_angle);
...
void ble_send_angle(float angle) {
    ...
    char buf[32];
    int len = snprintf(buf, sizeof(buf), "{\"angle\":%.2f}", angle);
