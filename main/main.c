...
#define VERTICAL_IN3  4
#define VERTICAL_IN4  5

#define STEP_COUNT 8
#define MOTOR_LOG_INTERVAL_MS 1000

#define VERTICAL_MAX_ANGLE 180.0f
#define VERTICAL_SAFE_ANGLE 90.0f
...
static StepperControl vertical_motor;

static void start_advertising(void);
void set_vertical_angle(float angle);
void manual_control_vertical(void);
void wifi_auto_control_steppers(void);
void offline_compute_steppers(void);
void control_vertical_fallback(void);
...
void vertical_motor_task(void *arg);
...
const char* mode_status_to_string(ModeStatus mode) {
    switch(mode) {
        case MODE_SAFE: return "SAFE";
        case MODE_WIFI_ONLINE: return "WIFI_ONLINE";
        case MODE_BLE_GPS: return "BLE_GPS_OFFLINE";
        case MODE_BLE_MANUAL: return "MANUAL_OFFLINE";
...
ModeStatus get_current_mode(void) {
    if (manual_control_active) return MODE_BLE_MANUAL;
    if (wifi_connected) return MODE_WIFI_ONLINE;
    if (ble_gps_valid) return MODE_BLE_GPS;
    return MODE_SAFE;
}
...
void update_mode_status(void) {
    ...
    if (new_mode != g_mode_status) {
        ESP_LOGI("MODE", "Mode changed: %s -> %s", 
            mode_status_to_string(g_mode_status),
            mode_status_to_string(new_mode));
...
void control_vertical_fallback(void) {
    set_vertical_angle(VERTICAL_SAFE_ANGLE);
...
void ble_send_system_status(void) {
    ...
    switch(g_mode_status) {
        ...
        case MODE_BLE_GPS:   mode_str = "ble_gps"; break;
        case MODE_BLE_MANUAL:    mode_str = "manual"; break;
        case MODE_SAFE:
    ...
    int len = snprintf(buf, sizeof(buf), 
        "STATUS:{\"vertical_angle\":%.2f,\"stepper_angle\":%.2f,\"mode\":\"%s\",\"wifi\":\"%s\",\"ble_mode\":\"%s\"}", 
        vertical_motor.current_angle, motor.current_angle, mode_str, wifi_status, ble_mode);
...
void control_loop_task(void *arg) {
    ...
    int64_t last_vertical_log = 0;
    int64_t last_motor_log = 0;
    float last_logged_angle = -1.0f;
    float last_logged_vertical_angle = -1.0f;
    ...
    while (1) {
        ...
        switch (g_mode_status) {
            case MODE_WIFI_ONLINE:
                wifi_auto_control_steppers();
                break;
            case MODE_BLE_GPS:
                offline_compute_steppers();
                break;
            case MODE_BLE_MANUAL:
                manual_control_vertical();
                break;
            case MODE_SAFE:
            default:
                control_vertical_fallback();
                break;
        }
        ...
        if (now - last_log_time > 1000) {
            ESP_LOGI("CONTROL", "Mode: %s", mode_status_to_string(g_mode_status));
            last_log_time = now;
        }
        
        if (now - last_vertical_log > 1000) {
            if (fabsf(vertical_motor.target_angle - vertical_motor.current_angle) > 1.0f) {
                ESP_LOGI("VERTICAL", "Moving: %.1f -> %.1f (diff: %.1f)", 
                        vertical_motor.current_angle, vertical_motor.target_angle,
                        vertical_motor.target_angle - vertical_motor.current_angle);
            }
            last_vertical_log = now;
        }

        if (now - last_motor_log > 1000) {
            if (fabsf(motor.current_angle - last_logged_angle) > 1.0f) {
            ...
            if (fabsf(vertical_motor.current_angle - last_logged_vertical_angle) > 1.0f) {
                ESP_LOGI("VERTICAL", "Vertical: current=%.1f°, target=%.1f°, steps=%ld, speed=%.1f", 
                    vertical_motor.current_angle, vertical_motor.target_angle, vertical_motor.current_step, vertical_motor.speed);
            }
            last_logged_angle = motor.current_angle;
            last_logged_vertical_angle = vertical_motor.current_angle;
            last_motor_log = now;
        }
...
void set_vertical_angle(float angle) {
    if (angle < 0) angle = 0;
    if (angle > VERTICAL_MAX_ANGLE) angle = VERTICAL_MAX_ANGLE;
    vertical_motor.target_angle = angle;
    vertical_motor.target_step = deg_to_steps(angle);
}
...
void motor_task(void *arg) {
    ESP_LOGI("MOTOR", "Horizontal motor task started");
    
    while (!ble_ready) {
        vTaskDelay(pdMS_TO_TICKS(100));
    }
    
    ESP_LOGI("MOTOR", "Horizontal motor task running");
    while (1) {
        stepper_run(&motor);
...
void vertical_motor_task(void *arg) {
    ESP_LOGI("VERTICAL", "Vertical motor task started");
    
    while (!ble_ready) {
        vTaskDelay(pdMS_TO_TICKS(100));
    }
    
    ESP_LOGI("VERTICAL", "Vertical motor task running");
    
    ESP_LOGI("VERTICAL_INIT", "Initial: current_angle=%.1f, target_angle=%.1f, current_step=%ld, target_step=%ld",
             vertical_motor.current_angle, vertical_motor.target_angle,
             vertical_motor.current_step, vertical_motor.target_step);
    
    while (1) {
        stepper_run(&vertical_motor);
        
        static int64_t last_debug = 0;
        int64_t now = esp_timer_get_time() / 1000;
        if (now - last_debug > 5000) { 
            ESP_LOGI("VERTICAL_DEBUG", "Angle: %.1f->%.1f, Steps: %ld->%ld, Speed: %.1f, Dir: %d",
                     vertical_motor.current_angle, vertical_motor.target_angle,
                     vertical_motor.current_step, vertical_motor.target_step,
                     vertical_motor.speed, vertical_motor.direction);
            last_debug = now;
        }
...
void wifi_auto_control_steppers(void) {
    int64_t last_observer_log = 0;
    int64_t last_sun_log = 0;

    if (!g_observer_valid || !g_sun_valid) {
        if (esp_timer_get_time() / 1000 - last_observer_log > 1000) {
            ESP_LOGW("WIFI_CONTROL", "No valid observer");
            last_observer_log = esp_timer_get_time() / 1000;
        }
        if (esp_timer_get_time() / 1000 - last_sun_log > 1000) {
            ESP_LOGW("WIFI_CONTROL", "No valid sun data");
            last_sun_log = esp_timer_get_time() / 1000;
        }
        control_vertical_fallback();
        return;
    }
    
    float vertical_target_angle = g_sun.altitude_deg;
    
    if (vertical_target_angle < 0) vertical_target_angle = 0;
    if (vertical_target_angle > VERTICAL_MAX_ANGLE) vertical_target_angle = VERTICAL_MAX_ANGLE;
    
    stepper_move_to_angle(&vertical_motor, vertical_target_angle);
...
void offline_compute_steppers(void) {
    ...
    set_vertical_angle(estimated_altitude);
    stepper_move_to_angle(&motor, estimated_azimuth);
    
    ESP_LOGD("OFFLINE_CONTROL", "Offline control: vertical=%.1f°, horizontal=%.1f°", 
...
void fetch_stellarium_status(const char *host_port)
{
    ...
    // Sun info
    snprintf(url, sizeof(url), "http://%s/api/objects/info?name=Saturn", host_port);
...
void app_main(void)
{
    ...
    stepper_init(&vertical_motor, VERTICAL_IN1, VERTICAL_IN2, VERTICAL_IN3, VERTICAL_IN4, 
                 MAX_SPEED_DEG_PER_SEC, MAX_ACCEL_DEG_PER_SEC2);
    stepper_set_current_angle(&vertical_motor, VERTICAL_SAFE_ANGLE);
    stepper_set_default_angle(&vertical_motor, VERTICAL_SAFE_ANGLE);

    ESP_LOGI("MAIN", "Horizontal motor: angle=%.1f, steps=%ld", 
             motor.current_angle, motor.current_step);
    ESP_LOGI("MAIN", "Vertical motor: angle=%.1f, steps=%ld", 
             vertical_motor.current_angle, vertical_motor.current_step);
    ...
    xTaskCreate(vertical_motor_task, "vertical_motor_task", 4096, NULL, 5, NULL);
    ...
    ESP_LOGI("MAIN", "Current mode: %s", mode_status_to_string(g_mode_status));
